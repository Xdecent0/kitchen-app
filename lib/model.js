...
/** Last-resort guess: strip the noise cashiers add, keep the longest word. */
function guessProduct(raw) {
  const cleaned = raw
    .replace(/\d+[.,]?\d*\s*(Г|КГ|МЛ|Л|ШТ|G|KG|ML|L)\b/giu, " ")
    .replace(/\d+[.,]?\d*\s*%/g, " ")
    .replace(/[^\p{L}\s]/gu, " ")
    .split(/\s+/)
    .filter((w) => w.length > 2);
  ...
  const word = cleaned.sort((a, b) => b.length - a.length)[0].toLowerCase();
...
export function freshness(item, now = today()) {
  ...
  if (expires == null || !item.shelfDays) return { left: null, share: null, tone: "calm" };

  const left = daysBetween(now, expires);
  const share = Math.max(0, Math.min(1, left / item.shelfDays));
...
/** A product is due when more time has passed since the last buy than its usual rhythm. */
export function isDue(product, history, now = today()) {
...
export function groupByAisle(entries, aisles) {
  ...
  for (const entry of entries) {
    const aisle = aisleOf(entry.product, aisles);
